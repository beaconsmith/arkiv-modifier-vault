...
import { ShieldCheck, Search, RefreshCw, ChevronDown, CheckCircle2, AlertTriangle, HelpCircle } from "lucide-react";
        ...
        <div className="flex items-center gap-3">
          ...
          <form onSubmit={handleHeaderSubmit} className="hidden sm:flex items-center relative">
            <input
              ...
              placeholder="Check reference (e.g. LP-4821)"
              disabled={isSearching}
              className="h-9 w-52 md:w-64 rounded-lg border border-slate-300 bg-slate-50 pl-9 pr-3 text-xs font-medium text-slate-900 placeholder:text-slate-400 focus:border-sky-600 focus:bg-white focus:outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:focus:border-sky-500"
            />
          ...
          {/* Secondary Action: Check another load */}
          {currentSearchId && (
            <button
              onClick={onReset}
